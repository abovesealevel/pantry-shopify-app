...
def order_fulfilled(data, shop_url):
    ...
    try:
        parsed = parsers.shopify_order_fulfillment(json.loads(data), shop_url)
    except KeyError as e:
        raise parsers.ParserException
    log.info(f'this is fulfilled order: {parsed}')

def product_created(data, shop_url):
    log.info(f'a product is created')
    try:
        parsed = parsers.shopify_order(json.loads(data), shop_url)
    except KeyError as e:
        raise parsers.ParserException
    log.info(f'a product is created: {parsed}')

def fulfillment_created(data, shop_url):
    log.info(f'a fulfillment was created')
    try:
        parsed = parsers.shopify_fulfillment(json.loads(data), shop_url)
    except KeyError as e:
        raise parsers.ParserException
    log.info(f'a product is fulfilled: {parsed}')
